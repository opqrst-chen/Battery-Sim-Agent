...
def black_box_function_real_vs_sim(exp_idx, configs, time_stamp,
    ...
    try:
        ...
        def extract_loss_value(loss_name, metric_name=None):
            try:
                loss_item = loss_list.get(loss_name, {})
                loss_value = loss_item.get("loss_value", -1e6)
                
                if isinstance(loss_value, dict):
                    if metric_name:
                        cycle_data = loss_value.get("cycle_1", {})
                        return cycle_data.get(metric_name, -1e6)
                    else:
                        cycle_data = loss_value.get("cycle_1", {})
                        if cycle_data:
                            return list(cycle_data.values())[0] if cycle_data else -1e6
                        return -1e6
                else:
                    return loss_value if isinstance(loss_value, (int, float)) else -1e6
            except Exception as e:
                logger.warning(f"[WARNING] Error extracting {loss_name}.{metric_name}: {e}")
                return -1e6
        
        objective_value = {
            "cycle_1_capacity_rmse": extract_loss_value("capacity", "rmse"),
            "cycle_1_capacity_mape": extract_loss_value("capacity", "mape"),
            "cycle_1_current_rmse": extract_loss_value("current", "rmse"),
            "cycle_1_current_mape": extract_loss_value("current", "mape"),
            "cycle_1_voltage_rmse": extract_loss_value("voltage", "rmse"),
            "cycle_1_voltage_mape": extract_loss_value("voltage", "mape"),
        }
        
        for key, value in objective_value.items():
            if not isinstance(value, (int, float)):
                logger.warning(f"[WARNING] Objective {key} is not a number: {value}, using default -1e6")
                objective_value[key] = -1e6
        
        return objective_value, loss_list, total_loss
