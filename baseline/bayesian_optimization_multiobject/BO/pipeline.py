...
try:
    from ax.api.generation_strategy import GenerationStrategy
except ImportError:
    # Ax>=1.1 moved GenerationStrategy under ax.generation_strategy.
    from ax.generation_strategy.generation_strategy import GenerationStrategy

try:
    from ax.core.generator import GenerationStep
except ImportError:
    # Ax>=1.1 exposes GenerationStep via generation_node.
    from ax.generation_strategy.generation_node import GenerationStep

try:
    from ax.modelbridge.registry import Generators
except ImportError:
    # Recent Ax versions renamed the registry module.
...
def run_bo_real_vs_sim(configs: BOConfigManager):
    ...
    try:
        ...
        ax_client = Client(random_seed=configs.get("exp.RANDOM_SEED"))
        import pprint
        # pprint.pprint(ax_parameters)
        ...
        ax_client.configure_optimization(
            objective = 'cycle_1_capacity_rmse, cycle_1_capacity_mape, cycle_1_current_rmse, cycle_1_current_mape, cycle_1_voltage_rmse, cycle_1_voltage_mape'
        )
        ...
        for exp_idx in tqdm(range(1, configs.get("BO.BO_search_trials") + 1)):
            ...
            trials = ax_client.get_next_trials(
                max_trials=1)
            trial_index = list(trials.keys())[0]
            # print(f"trials: {type(trials)}")
            # print(f"trials: {trials}")
            parameterization = trials[trial_index]
        ...
        try:
            frontier = ax_client.get_pareto_frontier()
            print("\n=== Pareto Frontier ===")
            if frontier:
                ...
                logger.warning("[WARNING] Pareto frontier is empty.")
                print("[WARNING] Pareto frontier is empty.")
        except Exception as e:
            logger.error(f"[ERROR] Failed to get Pareto frontier: {e}")
            print(f"[ERROR] Failed to get Pareto frontier: {e}")
            frontier = []
        ...
        try:
            ...
            logger.warning(f"[WARNING] Failed to get best parameters: {e}")
            print(f"[WARNING] Failed to get best parameters: {e}")
            restored_parameters = {}
            values = {}
                    ...
                    "pareto_frontier": [
                        {
                            ...
                            "parameters": {name_mapping.get(k, k): (v / scale_factor) for k, v in params.items()},
                            "metrics": metrics,
                            "arm_name": arm_name
                        }
                        for params, metrics, trial_index, arm_name in frontier
                    ] if frontier else [],
                    "experiment_type": "real_vs_sim",
...
def run_bo_sim_vs_sim(configs: BOConfigManager):
    ...
    try:
        ...
        try:
            analyses = ax_client.compute_analyses(display=True)
        except Exception as e:
            logger.warning(f"[WARNING] Failed to compute analyses: {e}")
