...
def plot_figure(ax,
    ...
    if isinstance(x, np.ndarray) or isinstance(x, list):
        ...
        ax.plot(y, label=label, alpha=0.8, linewidth=1.5)
    ax.set_title(title)
...
def plot_multiple_curves(sim,
    ...
    for cycle_idx in cycle_idxs:
        ...
        print(f"[INFO] length of time: {len(time)}")
        print(f"[INFO] length of voltage: {len(voltage)}")
        print(f"[INFO] length of current: {len(current)}")
        capacity = sim.solution.cycles[cycle_idx][
            "Discharge capacity [A.h]"].entries
        ...
        if ori_data:
            ...
            for i in range(len(periods)):
                ...
                context += f'Real battery {period_name} last {(time[period[1]]- time[period[0]]):.2f} seconds \n '
                detail_info['real'][period_name] = {
                    'voltage': voltage[period[0]:period[1] + 1],
                    'current': current[period[0]:period[1] + 1],
                    'time': time[period[0]:period[1] + 1],
                }
...
def plot_multiple_curves_sim_vs_sim(sim,
                         ori_data=None,
                         cycle_idxs=[1, 99, 299],
                         is_plot=True,
                         is_save=False,
    ...
    for cycle_idx in cycle_idxs:
        ...
        if ori_data:
            cycle_idx_str = f"cycle_{cycle_idx}"
            time = np.array(ori_data[cycle_idx_str]['time_in_s'])
            current = np.array(ori_data[cycle_idx_str]['current_in_A'])
            voltage = np.array(ori_data[cycle_idx_str]['voltage_in_V'])
            # charge_capacity = np.array(
            #     ori_data[cycle_idx_str]['charge_capacity_in_Ah'])
            # discharge_capacity = np.array(
            #     ori_data[cycle_idx_str]['discharge_capacity_in_Ah'])
            # # Qdlin = cycle_info[cycle_idx]['Qdlin']
            # discharge_mask = current < 0
            # charge_mask = current > 0
