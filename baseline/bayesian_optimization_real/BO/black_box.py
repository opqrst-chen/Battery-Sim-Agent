...
def black_box_function_real_vs_sim(exp_idx, configs, time_stamp,
    ...
    try:
        real_data_path = configs.get("exp.REAL_VS_SIM.real_world_data_path")
        ...
        try:
            ...
            context, detail_info = plot_multiple_curves(
                ...
                model=configs.get("pybamm.MODEL_NAME", ""),
                param=configs.get("pybamm.DEFAULT_PARAMS_SET", ""),
                id=f'sim_exp_{exp_idx}')
        except Exception as e:
            logger.info(f"[Error] in PyBaMM simulation: {e}")
            # Penalize this trial but keep return shape consistent in finally
            total_loss = 1e6
            loss_list = {
                k: {
                    "loss_name": k,
                ...
                for k in configs.get("BO.loss_setting.loss_type").keys()
            }
